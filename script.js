  ...
  users.forEach((user) => {
    user.task.forEach((task) => {
      ...
      row.innerHTML = `
        ...
        <td><button class="edit-button" data-task-id="${
          task.id
        }" style="margin: 0.5rem 0.5rem ;" >Edit</button>
        <button class="timer-button" data-task-id="${
          task.id
        }" style="margin: 0.5rem 0.5rem ;">${
        task.isStarted ? "End" : "Start"
      } Timer</button>
        </td>
      `;
...
taskTableBody.addEventListener("click", function (event) {
  ...
  if (
    ...
    target.classList.contains("timer-button")
  ) {
    const taskId = target.getAttribute("data-task-id");
